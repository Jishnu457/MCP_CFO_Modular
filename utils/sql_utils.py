"""
SQL utilities for query processing and validation
"""
import re
import structlog
import sqlparse
...
class SQLUtils:
    """SQL-specific utility functions"""
    ...
    def validate_group_by_syntax(sql: str) -> tuple[str, str]:
        ...
        try:
            ...
            select_columns = SQLUtils.parse_select_columns(select_part)
            group_by_columns = [col.strip() for col in group_by_part.split(',') if col.strip()]
            
            # Find missing columns
            missing_columns = []
            for sel_col in select_columns:
                if not SQLUtils.is_column_in_group_by(sel_col, group_by_columns):
    ...
    def clean_generated_sql(sql_text: str) -> str:
        ...
        sql = SQLUtils.remove_sql_comments(sql)
        ...
        for line in lines:
            ...
            if line.upper().startswith('SELECT'):
                ...
                sql_keywords = ['FROM', 'WHERE', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'ON', 
                               'GROUP', 'HAVING', 'ORDER', 'AND', 'OR', 'CASE', 'WHEN', 
                               'THEN', 'ELSE', 'END']
        ...
        if sql:
            ...
            problematic_patterns = ['FROM FROM', ', FROM', 'SELECT FROM', 'WHERE FROM']
            if any(issue in sql_upper for issue in problematic_patterns):
                return ""
                
            # Check for incomplete statements
            incomplete_endings = ['FROM', 'SELECT', 'WHERE', 'AND', 'OR', 'JOIN', 'ON', 'GROUP BY']
            if any(sql_upper.endswith(keyword) for keyword in incomplete_endings):
                return ""
    ...
    def sanitize_sql(sql: str) -> str:
        ...
        try:
            ...
            sql, group_by_msg = SQLUtils.validate_group_by_syntax(sql)
    ...
    def extract_filters_from_sql(sql: str) -> list[str]:
        """Extract WHERE conditions from SQL for context preservation"""
        if not sql:
            return []
